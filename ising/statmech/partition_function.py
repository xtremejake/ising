...
class PartitionGenerator():
    '''
    Class for generating partition functions to be used in Ising / nearest-neighbor models.

    This class generates sequences of partiiton functions in accordance with supplied terminal and intrinsic units.
    '''
    def __init__(self, model, units=['R', 'NR', 'NRC', 'RC'], intrinsic='R', max_length=2, min_length=1, num_cpus=2):
        self._supported_models = {'heteropolymer', 'homopolymer'} # define the supported model types
        self.model = model
    ...
    def _extend_names(self):
        ...
        Accepts: None
        Returns: str[] - extended name representations
        '''
        extended_names = []
        for u in self.units:
            # define symbols
    ...
    def _calculate(self):
        '''
        Calculates the partition function associated with each name

        Accepts: None
        Returns: <class PartitionFunction>[] - list of PartitionFunction() classes
        '''
